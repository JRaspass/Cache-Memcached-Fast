...
my @extra_keys = @keys;
for ( 1 .. count ) {
    splice( @extra_keys, int( rand( @extra_keys + 1 ) ), 0,
        "no_such_key-$_" );
}
...
SKIP: {
    skip "memcached 1.2.4 is required for cas/gets/append/prepend commands",
        27
...
SKIP: {
    ...
    ok( $Memd::memd->gat( undef, $key ),
        'gat expire_time : $expire -> undef'
    );
    sleep $wait_expire;
    ...
    my $res
        = $Memd::memd->gat_multi( $expire, 'key1', 'key2', 'no-such-key' );
    isa_ok( $res, 'HASH' );
    is_deeply(
        $res,
        {   'key1' => 'key1_value',
